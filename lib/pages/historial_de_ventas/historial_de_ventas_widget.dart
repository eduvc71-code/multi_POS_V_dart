...
class _HistorialDeVentasWidgetState extends State<HistorialDeVentasWidget> {
  ...
  Future<void> _loadVentas() async {
    ...
    try {
      final list = await SupabaseService.instance.readAllVentas(empresaId);
      if (mounted) {
        setState(() {
          _ventas = list;
          _isLoading = false;
      ...
      debugPrint('Error al cargar ventas desde Supabase: $e');
      if (mounted) {
        setState(() {
          _isLoading = false;
        });
  ...
  Future<void> _handleAnularVenta(Map<String, dynamic> venta) async {
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text('Total: ${formatBs((venta['total'] as num).toDouble())}'),
    ...
    if (confirm == true) {
      try {
        ...
        await SupabaseService.instance.processReturnSale(
          ventaId: ventaId,
          empresaId: empresaId,
          motivo: motivo,
        );
                            ...
                            if (_isLoading)
                              ...
                              Column(
                                ...
                                children: _ventas.map((v) {
                                  final int id = v['id'];
                                  final double total = (v['total'] as num)
                                      .toDouble();
                                  final String metodo =
                                  ...
                                  Color statusColor = FlutterFlowTheme.of(
                                    context,
                                  ).success;
                                  if (estado == 'ANULADA') {
                                    statusColor = FlutterFlowTheme.of(
                                      context,
                                    ).error;
                                  ...
                                  return Padding(
                                    ...
                                    child: InkWell(
                                      ...
                                      child: SaleRowWidget(
                                        folio:
                                            'V-${id.toString().padLeft(6, '0')}',
                                        method: metodo,
                                        statusColor: statusColor,
                                        time: fecha.length > 16
