...
class DatabaseHelper {
  ...
  Future<int> registerFullBusiness({
    ...
    // Si se activa el inventario estándar y el tipo es Tienda, intentar cargar productos de Open Food Facts (Bolivia)
    List<Map<String, dynamic>> items = [];
    if (populateStandardInventory) {
      if (businessType == 'Tienda') {
        try {
          final offBoliviaItems =
              await OpenFoodFactsService.fetchBoliviaProducts();
          if (offBoliviaItems.isNotEmpty) {
            items = offBoliviaItems;
          }
        } catch (e) {
          debugPrint('Error al obtener productos de Open Food Facts para Tienda: $e');
        }
      }

      // Si no es Tienda o si falló la carga remota, usar la librería base local
      if (items.isEmpty) {
    ...
    return await db.transaction((txn) async {
      ...
      if (populateStandardInventory && items.isNotEmpty) {
        final random = Random();

        // Mezclar índices para seleccionar 10 productos aleatorios con precio y stock inicial
