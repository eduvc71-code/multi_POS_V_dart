...
class _ClientesYCreditosWidgetState extends State<ClientesYCreditosWidget> {
  ...
  Future<void> _loadClientes() async {
    ...
    try {
      final list = await SupabaseService.instance.readAllClientes(empresaId);
      if (mounted) {
        setState(() {
          _clientes = list;
          _isLoading = false;
      ...
      debugPrint('Error al cargar clientes desde Supabase: $e');
      if (mounted) {
        setState(() {
          _isLoading = false;
        });
  ...
  Future<void> _handleAgregarCliente() async {
    ...
    if (confirm == true && nombre.trim().isNotEmpty) {
      ...
      await SupabaseService.instance.createCliente(
        nombre: nombre,
        nit: nit,
        telefono: telefono,
        direccion: direccion,
  ...
  Future<void> _handleAbono(Map<String, dynamic> cliente) async {
        ...
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                Text('Deuda Actual: ${formatBs(deudaActual)}'),
    ...
    if (confirm == true && montoAbono > 0) {
      try {
        ...
        await SupabaseService.instance.processAbonoCredito(
          clienteId: clienteId,
          monto: montoAbono,
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Abono de ${formatBs(montoAbono)} registrado exitosamente.',
              ),
              backgroundColor: FlutterFlowTheme.of(context).success,
            ),
          );
      ...
      child: GestureDetector(
        ...
        bottomNavigationBar: wrapWithModel(
          ...
          child: BottomNavWidget(
            child: () => const BottomNavChild3Widget(),
          ),
        ),
        body: Column(
                                ...
                                Expanded(
                                  ...
                                  child: wrapWithModel(
                                    ...
                                    child: CreditStatWidget(
                                      ...
                                      value: formatBs(_totalPorCobrar),
                                    ),
                                  ),
                                ),
                                Expanded(
