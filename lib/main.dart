...
void main() async {
  ...
  // Inicializar Supabase en el inicio de la app
...
class _AppInitializerWidgetState extends State<AppInitializerWidget> {
  ...
  Future<void> _checkAppState() async {
    try {
      ...
      final hasCompany = empresaId != null && empresaId > 0;

      if (!mounted) return;

      if (!hasCompany) {
        // No hay empresa registrada -> Ir al registro
        context.goNamed(RegistroDeNegocioWidget.routeName);
      } else {
        // Ya hay una empresa registrada -> Iniciar en Login
