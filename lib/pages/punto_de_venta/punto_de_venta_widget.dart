...
class _PuntoDeVentaWidgetState extends State<PuntoDeVentaWidget> {
  ...
  Future<void> _handleAddToCart(Producto producto) async {
    ...
    if (_empresaTipo == 'Farmacia' && producto.id != null) {
      final loteFEFO = await SupabaseService.instance
          .readLoteFEFOActivo(producto.id!, empresaId);
    ...
    if (error.isNotEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('✓ ${producto.nombre} agregado al carrito (FEFO)'),
  ...
  void _updateCartItemUnidad(PosCartItem item, String unidad) {
    item.unidadVenta = unidad;
    if (item.productoId != null) {
      final p = _model.searchResults.firstWhere(
                      ...
                      child: TextField(
                        ...
                        onChanged: (q) async {
                          if (q.trim().isEmpty) {
                            ...
                            setSheetState(
                              () => filteredProducts = _model.searchResults,
                            );
                          }
                        },
                      ),
                                              ...
                                              children: [
                                                ...
                                                if (qty >= p.stock)
                                                  Padding(
                                                    ...
                                                    child: Text(
                                                      'Stock máximo disponible: ${p.stock} u.',
                                                      style: const TextStyle(
                                                        color: Colors.orange,
                                                        fontSize: 11,
                                                      ),
        ...
        if (productLib != null) {
          ...
          final confirm = await showDialog<bool>(
            ...
            builder: (context) => AlertDialog(
              ...
              content: Text(
                'Se encontró "${productLib['nombre']}" en la librería de $businessType.\n\n¿Desea agregarlo al inventario y a la venta actual?',
              ),
              actions: [
                TextButton(
          ...
          // Buscar en Open Food Facts (Bolivia / América Latina)
          if (!currentContext.mounted) return;
          ...
          if (offProduct != null) {
            ...
            if (confirm == true) {
              ...
              await SupabaseService.instance.createProducto(nuevoProducto, empresaId);
              final p = await SupabaseService.instance.readProductoByCodigo(
                code,
                empresaId,
              );
              if (p != null) _handleAddToCart(p);
  ...
  Future<void> _handleCheckout() async {
    ...
    final montoRecibidoCtrl = TextEditingController(
      text: _model.total.toStringAsFixed(2),
    );
        ...
        return StatefulBuilder(
          builder: (dialogCtx, setStateDialog) {
            ...
            final double recibido =
                double.tryParse(montoRecibidoCtrl.text) ?? totalVenta;
            final double cambio = recibido >= totalVenta
                ? recibido - totalVenta
                : 0.0;
                  ...
                  children: [
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: const [
                        DropdownMenuItem(
                          value: 'EFECTIVO',
                          child: Text('💵 Efectivo (Calculadora de Vueltas)'),
                        ),
                        DropdownMenuItem(
                          value: 'TRANSFERENCIA',
                          child: Text('📲 Transferencia QR / Banco'),
                        ),
                        DropdownMenuItem(
                          value: 'QR_EFECTIVO',
                          child: Text('💵📲 QR + Efectivo'),
                        ),
                        DropdownMenuItem(
                          value: 'TARJETA',
                        ...
                        DropdownMenuItem(
                          value: 'CREDITO',
                          child: Text('📑 Venta a Crédito / Fiado'),
                        ),
                      ],
                      onChanged: (val) {
                        if (val != null) {
                    ...
                    if (selectedMetodo == 'TRANSFERENCIA' ||
                        ...
                        onPressed: () async {
                          ...
                          if (photo != null) {
                            setStateDialog(() {
                              ...
                              comprobanteCtrl.text =
                                  'QR-${DateTime.now().millisecondsSinceEpoch.toString().substring(6)}';
                            });
                          }
                        },
                        icon: const Icon(Icons.camera_alt_rounded, size: 18),
                        label: Text(
                          comprobanteFotoPath == null
                              ? '📷 Tomar Foto Comprobante QR'
                              : '✓ Foto Capturada',
                          style: const TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 12,
                      ...
                      if (comprobanteFotoPath != null) ...[
                        ...
                        Text(
                          'Comprobante ID: ${comprobanteCtrl.text}',
                          style: const TextStyle(
                            fontSize: 11,
                            color: Colors.green,
                            fontWeight: FontWeight.bold,
    ...
    if (result != null) {
      try {
        ...
        ScaffoldMessenger.of(currentContext).showSnackBar(
          SnackBar(
            content: Text(
              '¡Venta #$ventaId realizada con éxito! (${result['metodo']})',
            ),
            backgroundColor: FlutterFlowTheme.of(currentContext).success,
          ),
        );
      } catch (e) {
        if (!currentContext.mounted) return;
        ScaffoldMessenger.of(currentContext).showSnackBar(
          SnackBar(
            ...
            backgroundColor: FlutterFlowTheme.of(currentContext).error,
          ),
        );
      }
    }
                            ...
                            ? Center(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    const SizedBox(height: 8),
                                    const Text(
                                      'Sin productos en el cobro actual',
                                      style: TextStyle(
                                        color: Colors.grey,
                                        fontWeight: FontWeight.bold,
                                        fontSize: 13,
                                    ...
                                    const SizedBox(height: 4),
                                    const Text(
                                      'Use [📦 Catálogo], [+ Ítem] o [📷 Escáner] para agregar',
                                      style: TextStyle(
                                        color: Colors.grey,
                                        fontSize: 11,
                                      ),
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              if (_empresaTipo == 'Farmacia') ...[
                                                Row(
                                                  mainAxisAlignment: MainAxisAlignment.center,
                                                  children: ['Caja', 'Blíster', 'Pastilla'].map((u) {
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: Text(
                          'COBRAR AHORA · ${formatBs(_model.total)}',
                          style: const TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 15,
                          ),
